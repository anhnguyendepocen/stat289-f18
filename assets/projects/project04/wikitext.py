...
def _get_doc_page(wcorp, topic_names, clust_names):
    ...
    for idx in range(wcorp.meta.shape[0]):
        ...
        if wcorp.meta.first_img[idx] is not '':
            SubElement(xml_td, 'img', src=wcorp.meta.first_img[idx],
...
def _add_country_flags(wcorp, idx, eli):
    if 'no' in wcorp.meta.langs.values[idx]:
        SubElement(eli, 'img', src="https://upload.wikimedia.org/wikipedia/"
                                   "commons/thumb/d/d9/Flag_of_Norway.svg/"
                                   "275px-Flag_of_Norway.svg.png",
                   style="width: 20px; border:1px solid black")
    if 'ru' in wcorp.meta.langs.values[idx]:
        SubElement(eli, 'img', src="https://upload.wikimedia.org/wikipedia/"
                                   "en/thumb/f/f3/Flag_of_Russia.svg/510px-"
                                   "Flag_of_Russia.svg.png",
                   style="width: 20px; border:1px solid black")
    if 'es' in wcorp.meta.langs.values[idx]:
        SubElement(eli, 'img', src="https://upload.wikimedia.org/wikipedia/"
                                   "en/thumb/9/9a/Flag_of_Spain.svg/300px-"
